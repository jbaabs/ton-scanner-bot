...
# Background collection: volatile market data can be refreshed frequently, while
# expensive metadata/holder calls are refreshed less often.
BACKGROUND_FULL_SCAN_SECONDS = max(120, int(os.getenv("BACKGROUND_FULL_SCAN_SECONDS", "300")))
BACKGROUND_CONCURRENCY = max(2, min(12, int(os.getenv("BACKGROUND_CONCURRENCY", "6"))))
SNAPSHOT_MIN_GAP_SECONDS = max(20, int(os.getenv("SNAPSHOT_MIN_GAP_SECONDS", "45")))
BACKGROUND_REPORT_CACHE: dict[str, tuple[float, dict]] = {}
...
async def alert_watcher():
    """Efficient background collector for watchlist + alerts.

    - One persistent HTTP session instead of reconnecting every cycle.
    - Bounded concurrency across tokens.
    - Full scan (holders/metadata) every few minutes.
    - Fast market refresh between full scans.
    - Every watched pool is registered with the live TON stream.
    """
    await asyncio.sleep(5)
    timeout = aiohttp.ClientTimeout(total=12)
    connector = aiohttp.TCPConnector(limit=40, limit_per_host=12, ttl_dns_cache=300)
    ...
    async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
        while True:
            ...
            try:
                ...
                async def collect_one(address: str):
                    async with semaphore:
                        cached = BACKGROUND_REPORT_CACHE.get(address)
                        cached_ts, old_report = cached if cached else (0.0, None)
                        age = time.monotonic() - cached_ts if cached else float("inf")

                        try:
                            if old_report is None or age >= BACKGROUND_FULL_SCAN_SECONDS:
                                report = await scan_token(session, address)
                                BACKGROUND_REPORT_CACHE[address] = (time.monotonic(), report)
                            else:
                                report = await fast_refresh_report(session, old_report)
                                # Preserve the timestamp of the last *full* scan so holder
                                # refresh cadence is independent from fast market refreshes.
                                BACKGROUND_REPORT_CACHE[address] = (cached_ts, report)

                            dex = report.get("dex_data") or {}
                            _register_live_pool(
                                dex.get("chart_pair_address") or dex.get("pair_address")
                            )
                            save_token_snapshot(report)
                            return address, report, None
                        except Exception as exc:
                            return address, None, exc

                results = await asyncio.gather(
                    *(collect_one(address) for address in addresses),
                    return_exceptions=False,
                )

                for address, report, error in results:
                    if error is not None or not report:
                        logger.error("Background GRX snapshot failed for %s: %s", address, error)
                        continue
            ...
            await asyncio.sleep(max(1.0, ALERT_CHECK_SECONDS - elapsed))
...
def save_token_snapshot(report: dict) -> None:
    ...
    conn = sqlite3.connect(DB_PATH)
    # Avoid duplicate rows caused by refresh taps landing on the same collector tick.
    last = conn.execute(
        "SELECT snapshot_ts FROM token_snapshots WHERE token_key=? ORDER BY snapshot_ts DESC LIMIT 1",
        (token_key,),
    ).fetchone()
    if last and now_ts - int(last[0]) < SNAPSHOT_MIN_GAP_SECONDS:
        conn.close()
        return

    holder_value = info.get("holders_count")
    holder_value = int(holder_value) if holder_value not in (None, "") else None
...
def get_snapshot_after(report: dict, seconds_ago: int) -> dict | None:
    """Return a snapshot close to the requested historical boundary.

    A stale snapshot hours away must never masquerade as a 5m/10m comparison.
    """
    token_key = _history_key(report)
    if not token_key:
        return None
    ...
    # 5m tolerates ~2m either side; longer windows get a proportional allowance.
    tolerance = max(120, min(600, int(seconds_ago * 0.35)))
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    row = conn.execute(
        """
        ...
        LIMIT 1
        """,
        (token_key, target - tolerance, target + tolerance, target),
    ).fetchone()
    conn.close()
    return dict(row) if row else None
...
def format_grx_stats(report: dict) -> str:
    ...
    if buy_pressure is None:
        ...
        total = buys + sells
        # Tiny samples produce silly-looking 0%/100% pressure. Require enough
        # observations before treating the ratio as a signal.
        buy_pressure = (buys / total * 100.0) if total >= 5 else None
