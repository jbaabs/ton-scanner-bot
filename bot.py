...
async def handle_address(message: Message):
    # TEMPORARY CUSTOM EMOJI ID COLLECTOR
    # Send one or more Telegram custom emojis to this bot in a single message.
    # Their custom_emoji_id values will be returned in the same order.
    custom_entities = [
        entity
        for entity in (message.entities or [])
        if getattr(entity.type, "value", str(entity.type)) == "custom_emoji"
    ]

    if custom_entities:
        results = []
        for i, entity in enumerate(custom_entities, start=1):
            emoji_id = getattr(entity, "custom_emoji_id", None)
            if emoji_id:
                results.append(f"{i}. <code>{html.escape(str(emoji_id))}</code>")

        if results:
            await message.answer(
                "<b>Custom Emoji IDs</b>\n\n"
                + "\n".join(results)
                + "\n\n<i>Send this reply back to ChatGPT.</i>"
            )
            return
