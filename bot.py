...
def format_token_report(report: dict, show_info: bool = False, show_holders: bool = False) -> str:
    ...
    if dex.get("price_native"):
        ...
        lines.append(f"  Native: {native} {quote_sym}")
    ...
    if changes:
        lines.append(f"  Change: {' | '.join(html.escape(c) for c in changes)}")
    ...
    if dexes:
        dexes_safe = [html.escape(str(d)) for d in dexes]
        lines.append(f"DEX: {', '.join(dexes_safe)} ({pair_count} pair{'s' if pair_count != 1 else ''})")
    ...
    if dex.get("dex_url"):
        ...
        lines.append(f'<a href="{safe_url}">View on DexScreener</a>')
    ...
    if show_info:
        ...
        if supply:
            try:
                ...
                lines.append(f"Total Supply: {_fmt_num(human)} {html.escape(str(symbol))}")
            except (ValueError, TypeError):
                pass

        lines.append(f"Mintable: {'Yes' if info.get('mintable') else 'No'}")
    else:
        lines.append("<b>Token Info</b>  ▸ tap below to expand")
    lines.append("")
    ...
    if holder_list:
        if show_holders:
            ...
            lines.append("<b>Top Holders</b>  ▸ tap below to expand")
        lines.append("")
...
def build_report_keyboard(key: str, show_info: bool, show_holders: bool) -> InlineKeyboardBuilder:
...
async def handle_address(message: Message):
    ...
    try:
        ...
        keyboard = build_report_keyboard(key, show_info=False, show_holders=False)
...
async def handle_toggle(callback: CallbackQuery):
    """Handle Token Info / Top Holders expand-collapse button taps."""
    try:
    ...
    keyboard = build_report_keyboard(key, entry["show_info"], entry["show_holders"])
    try:
