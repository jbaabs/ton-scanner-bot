...
def format_token_report(
    ...
    mcap = html.escape(_fmt_usd(dex.get("market_cap")))
    liq = html.escape(_fmt_usd(dex.get("liquidity_usd")))
    ...
    renounced_text = "Mintable" if mintable else "Renounced"

    lines = [
        f"<b>{symbol} • {name}</b>",
        f"<code>{safe_address}</code>",
        "",
        f"💰 Price: <b>{price}</b>  •  {_ce('percent', '%')} 1H: <b>{h1}</b>",
        f"📈 MCap: <b>{mcap}</b>  •  🔥 24H: <b>{h24}</b>",
        f"🌱 Age: <b>{age}</b>  •  🔑 <b>{renounced_text}</b>",
        f"💧 LP: <b>{liq}</b>  •  📊 Vol: <b>{vol}</b>",
        "",
        f"{_ce('buy', '🟢')} Buys: <b>{buys:,}</b> ({buy_pct:.0f}%)"
        f"   {_ce('sell', '🔴')} Sells: <b>{sells:,}</b> ({sell_pct:.0f}%)",
    ]

    holder_list = holders.get("holders") or []
    top_concentration = holders.get("top_concentration")
    top_text = f"{top_concentration:.2f}%" if top_concentration is not None else "N/A"
    lines += [
        "",
        f"👥 Holders: <b>{_fmt_num(info.get('holders_count'))}</b>  •  Top 10: <b>{html.escape(top_text)}</b>",
    ]
    ...
    if holder_list:
        ...
        for i, holder in enumerate(holder_list[:6], 1):
            ...
            icon = _ce("wallet", "👛")
            if wallet_address:
                icon = _linked_ce("wallet", "👛", f"https://tonviewer.com/{wallet_address}")
            holder_bits.append(f"{i}. {icon} <b>{html.escape(pct_text)}</b>")
        for i in range(0, len(holder_bits), 2):
            lines.append("  |  ".join(holder_bits[i:i + 2]))

    if bonding and not bonding.get("bonded", False):
        percent = bonding.get("percent")
        bar = _progress_bar(percent, size=10)
        pct_text = f"{percent:.1f}%" if percent is not None else "N/A"
        lines += [
            "",
            "<b>🧨 Bonding Curve</b>",
            f"<code>{html.escape(bar)}</code> <b>{html.escape(pct_text)}</b>",
    ...
    # Branded, clickable service/social links.
    links = _collect_links(report)
    link_parts = []
    seen_labels = set()
    for label, url in links:
        if label == "DS" and "DS" not in seen_labels:
            link_parts.append(f"{_linked_ce('dexscreener', '📊', url)} <a href=\"{html.escape(url, quote=True)}\">DexScreener</a>")
            seen_labels.add("DS")
        elif label == "TG" and "TG" not in seen_labels:
            link_parts.append(f"{_linked_ce('social', '💬', url)} <a href=\"{html.escape(url, quote=True)}\">Telegram</a>")
            seen_labels.add("TG")
        elif label == "X" and "X" not in seen_labels:
            link_parts.append(f"𝕏 <a href=\"{html.escape(url, quote=True)}\">X</a>")
            seen_labels.add("X")
        elif label == "TV" and "TV" not in seen_labels:
            link_parts.append(f"{_linked_ce('wallet', '👛', url)} <a href=\"{html.escape(url, quote=True)}\">TON Viewer</a>")
            seen_labels.add("TV")
        elif label == "WEB" and "WEB" not in seen_labels:
            link_parts.append(f"🌐 <a href=\"{html.escape(url, quote=True)}\">Website</a>")
            seen_labels.add("WEB")
    ...
    if show_info:
        lines += [
            ...
            f"Verification: <b>{html.escape(str(info.get('verification') or 'none'))}</b>",
            f"Contract: <code>{safe_address}</code>",
        ]

    if holder_list and show_holders:
        lines += ["", "<b>👥 Top Holders — Full View</b>"]
        for i, holder in enumerate(holder_list[:10], 1):
            pct = holder.get("percentage")
            pct_text = f"{pct:.2f}%" if pct is not None else "N/A"
            wallet_address = str(holder.get("address") or "").strip()
            short = html.escape(_short_address(wallet_address))
            if wallet_address:
                wallet_url = f"https://tonviewer.com/{wallet_address}"
                lines.append(
                    f"{i}. {_linked_ce('wallet', '👛', wallet_url)} "
                    f"<a href=\"{html.escape(wallet_url, quote=True)}\">{short}</a> — <b>{html.escape(pct_text)}</b>"
                )
            else:
    ...
    if first_scan:
        scanner_name_raw = str(first_scan.get("scanner_name") or DEFAULT_SCANNER_LABEL)
        scanner_name = html.escape(scanner_name_raw)
        scanner_id = first_scan.get("scanner_id")
        if scanner_id:
            scanner_name = f'<a href="tg://user?id={int(scanner_id)}">{scanner_name}</a>'
        scan_mc = html.escape(str(first_scan.get("scan_market_cap") or "N/A"))
        scan_age = html.escape(_fmt_scan_age(first_scan.get("scan_ts")))
        lines += [
            "",
            f"📞 First called: {scanner_name} <b>{scan_age} ago</b> • MCap <b>{scan_mc}</b>",
        ]
