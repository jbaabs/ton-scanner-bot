...
def init_db():
    ...
    # Snake mini-game scores. Kept as a distinct block/comment so this
    # addition stays easy to spot if this file and the web-server code
    # diverge again in the future.
    conn.execute(
        """
...
def build_report_card(ohlcv: list, report: dict, timeframe_label: str, token_icon_bytes: bytes | None = None, grx_watermark_bytes: bytes | None = None) -> bytes:
    ...
    box(.025,.460,.465,.135); box(.51,.460,.465,.135)
    fig.text(.045,.570,"MARKET PULSE",color=muted,fontsize=8,fontweight="bold")
    for j,(lab,val) in enumerate((("1H",h1),("6H",h6),("24H",h24))):
        y=.542-j*.027; fig.text(.05,y,lab,color=muted,fontsize=8.5); fig.text(.465,y,_fmt_pct(val),color=pc(val),fontsize=10,fontweight="bold",ha="right")
    fig.text(.53,.570,"YOUR FIRST SCAN",color=muted,fontsize=8,fontweight="bold")
    first=report.get("_viewer_first_scan")
    if first:
    ...
    else: fig.text(.53,.525,"First scan",color=muted,fontsize=10)
...
# This block keeps disappearing when bot.py gets edited from an older base
# that predates it — if this file stops having a working Snake game again,
# check here first: this whole section needs to exist for the game to work
# at all (it's what actually binds to a port and serves the game/API).
