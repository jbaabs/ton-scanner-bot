    ...
    while True:
        try:
            with sqlite3.connect(DB_PATH) as conn:
                ...
                rows=conn.execute("SELECT * FROM token_alerts WHERE active=1 ORDER BY token_address").fetchall()
            by_addr={}
            for r in rows: by_addr.setdefault(r["token_address"],[]).append(r)
            async with aiohttp.ClientSession() as session:
                for address, alerts in by_addr.items():
                    try: report=await scan_token(session,address)
                    except Exception: continue
                    dex=report.get("dex_data") or {}; price=_as_float(dex.get("price_usd")); mcap=_as_float(dex.get("market_cap")); ath=_as_float(dex.get("ath_market_cap"))
                    for a in alerts:
                        typ=a["alert_type"]; target=a["threshold"]; hit=False; current=None
                        if typ=="price_above" and price is not None: hit,current=price>=target,price
                        elif typ=="price_below" and price is not None: hit,current=price<=target,price
                        elif typ=="mcap_above" and mcap is not None: hit,current=mcap>=target,mcap
                        elif typ=="mcap_below" and mcap is not None: hit,current=mcap<=target,mcap
                        elif typ=="new_ath" and mcap is not None: hit,current=mcap>(a["baseline"] or ath or float("inf")),mcap
                        if hit:
                            label={"price_above":"Price crossed above","price_below":"Price crossed below","mcap_above":"Market cap crossed above","mcap_below":"Market cap crossed below","new_ath":"New ATH reached"}[typ]
                            target_txt=_money(target) if typ.startswith("mcap") else (_money(target) if target is not None else "")
                            try: await bot.send_message(a["user_id"], f"🚨 <b>GRX Alert</b>\n\n<b>{html.escape(a['token_symbol'] or 'Token')}</b>\n{label}{(' <b>'+target_txt+'</b>') if target_txt else ''}\nCurrent: <b>{_money(current)}</b>\n\n<code>{html.escape(address)}</code>")
                            except Exception: logger.exception("Could not deliver alert to %s",a["user_id"])
                            with sqlite3.connect(DB_PATH) as conn: conn.execute("UPDATE token_alerts SET active=0, triggered_ts=? WHERE id=?",(int(time.time()),a["id"]))
                    await asyncio.sleep(.25)
        except asyncio.CancelledError: raise
        except Exception: logger.exception("Alert watcher cycle failed")
